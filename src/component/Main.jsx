...
function Main() {
    ...
    const checkPaymentStatus = async (orderId, email) => {
        const tokens = [
            "b93b87-7195bc-2f74f2-29903f-930a8c",
            "225e3b-5843ec-ddb76d-a14f84-5c4741"
        ];
            ...
            try {
                ...
                if (data.status && data.results.status === "Success") {
                    ...
                    if (hasValidSubscription) {
                        alert("You have previously bought a subscription. Please log in for VIP access.");
                    }
                    if (!hasValidSubscription) {
                ...
                console.error(`Error checking payment status with token: ${token}`, err);
            }
        }
    
        if (!isSuccess) {
            alert('An error occurred while checking payment status with all available tokens.');
        }
    };
